 ...
 * 2) Якоря снабжения — любые клетки из V внутри зоны активной базы 6×6 (главная и каждый
 *    плацдарм). Так фронт ведёт себя как вторая база даже если центр (флаг) не закрашен
 *    (вода / пропуск при заливке).
 * 3) Компонента, пересекающаяся с этим множеством якорей — «base-connected», без таймера 20 с.
 * 4) Каждая остальная компонента — изолированный карман со своим groupId и 20 s на сервере.
 *
 * Если ни одна клетка баз не в V (все зоны пусты), все компоненты изолированы.
 */
...
/**
 * Клетки команды (из `vertices`), лежащие в любой активной базе 6×6: главная + плацдармы.
 * Это множество корней снабжения для изоляции и (на сервере) для BFS «связь с базой».
 *
 * @param {Set<string>} vertices
