 ...
 *
 * Правило: **та же** 8-связная достижимость, что у `computeBaseConnectedPixelKeysForTeam` на сервере:
 * семена — клетки V внутри каждой активной базы 6×6 (главная + плацдармы); полный BFS по V.
 * Если внутри базы ещё нет своих пикселей — семена дополняются клетками V, 8-соседними с любой клеткой прямоугольника базы
 * (иначе плацдарм ошибочно попадал в «карман» с таймером изоляции).
 */
...
/**
 * Все клетки V, 8-достижимые от **любой** активной базы команды (как серверный BFS «связь с базой»).
 *
 * @param {Set<string>} vertices
...
export function computeIsolatedTerritoryGroups(pixels, teams, pixelTeamFn, flagCellFromSpawnFn, spawnSize = 6) {
  ...
  for (const t of teams) {
    ...
    const mosRaw = Array.isArray(t.militaryOutposts) ? t.militaryOutposts : [];
    let hasMo = false;
    for (let mi = 0; mi < mosRaw.length; mi++) {
      const o = mosRaw[mi];
    ...
    const reachable = computeSupplyReachableFromTeamBases(
      vertices,
      hasMain ? t.spawnX0 : undefined,
      hasMain ? t.spawnY0 : undefined,
      mosRaw,
      spawnSize
    );
