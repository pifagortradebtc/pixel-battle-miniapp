 ...
 *
 * **Изоляция и коллапс через ~20 с** (`computeIsolatedTerritoryGroups`): «снабжение» от **главной** 6×6 **и** всех
 * военных плацдармов (`computeSupplyReachableFromTeamBases`), как у линии размещения пикселей. Карман и таймер — только
 * у компонентов, не 8-связных ни с одной из этих баз.
 *
 * **Размещение пикселей** на сервере: тот же набор якорей — `computeBaseConnectedPixelKeysForTeam`.
 *
 * Внутри главной базы 6×6 или плацдарма 2×2 клетки не попадают в список кармана — см. `cellKeyInsideAnyTeamBaseRect`.
 */

import { MILITARY_OUTPOST_SIZE, FLAG_SPAWN_SIZE } from "./flag-capture.js";
 ...
 * Пустой `militaryOutposts` — только главная 6×6 как корень (см. `computeSupplyReachableForIsolation`).
 *
 * @param {Set<string>} vertices
 * @param {number} spawnX0
 * @param {number} spawnY0
 * @param {unknown} militaryOutposts
 * @param {number} [spawnSize=6] сторона главной базы
 * @param {number} [militaryFootprintSize=2] сторона каждого плацдарма (как MILITARY_OUTPOST_SIZE)
 * @returns {Set<string>}
 */
export function computeSupplyReachableFromTeamBases(
  vertices,
  spawnX0,
  spawnY0,
  militaryOutposts,
  spawnSize = 6,
  militaryFootprintSize = MILITARY_OUTPOST_SIZE
) {
  const SMain = spawnSize | 0;
  const SMil = militaryFootprintSize | 0;
  if (SMain < 1 || SMil < 1 || !vertices?.size) return new Set();
  const reachable = new Set();
  const stack = [];
  const neighBuf = [];
  const addRectInteriorSeeds = (x0, y0, side) => {
    const ox = x0 | 0;
    const oy = y0 | 0;
    const S = side | 0;
  ...
  if (typeof spawnX0 === "number" && typeof spawnY0 === "number") {
    addRectInteriorSeeds(spawnX0, spawnY0, SMain);
  }
  ...
  for (let mi = 0; mi < mos.length; mi++) {
    ...
    const mw =
      typeof o.w === "number" && o.w > 0 ? o.w | 0 : typeof o.h === "number" && o.h > 0 ? o.h | 0 : SMil;
    addRectInteriorSeeds(o.x0, o.y0, mw);
  }
  ...
  const addTouchSeedsFromRect = (x0, y0, side) => {
    const ox = x0 | 0;
    const oy = y0 | 0;
    const S = side | 0;
    for (let y = oy; y < oy + S; y++) {
  ...
  if (typeof spawnX0 === "number" && typeof spawnY0 === "number") {
    addTouchSeedsFromRect(spawnX0, spawnY0, SMain);
  }
  for (let mi = 0; mi < mos.length; mi++) {
    const o = mos[mi];
    if (!o || typeof o.x0 !== "number" || typeof o.y0 !== "number") continue;
    const mw =
      typeof o.w === "number" && o.w > 0 ? o.w | 0 : typeof o.h === "number" && o.h > 0 ? o.h | 0 : SMil;
    addTouchSeedsFromRect(o.x0, o.y0, mw);
  }
  while (stack.length) {
    const cur = stack.pop();
    const neigh = neighborKeysInSet8(cur, vertices, neighBuf);
...
/**
 * Достижимость **только от главной** 6×6 (без плацдармов). Для игровой изоляции не используется — см. `computeIsolatedTerritoryGroups`.
 *
 * @param {Set<string>} vertices
 * @param {number} spawnX0
 * @param {number} spawnY0
 ...
 * @returns {Set<string>}
 */
export function computeSupplyReachableForIsolation(vertices, spawnX0, spawnY0, spawnSize = 6) {
  return computeSupplyReachableFromTeamBases(vertices, spawnX0, spawnY0, [], spawnSize);
}
...
function cellKeyInsideAnyTeamBaseRect(key, t, spawnSize) {
  ...
  if (!p) return false;
  const S = spawnSize | 0;
  const inRectSized = (x0, y0, side) => {
    const ox = x0 | 0;
    const oy = y0 | 0;
    const ss = side | 0;
    return p.x >= ox && p.x < ox + ss && p.y >= oy && p.y < oy + ss;
  };
  if (typeof t.spawnX0 === "number" && typeof t.spawnY0 === "number" && inRectSized(t.spawnX0, t.spawnY0, S))
    return true;
  ...
  for (let i = 0; i < mos.length; i++) {
    ...
    if (!o || typeof o.x0 !== "number" || typeof o.y0 !== "number") continue;
    const mw =
      typeof o.w === "number" && o.w > 0 ? o.w | 0 : typeof o.h === "number" && o.h > 0 ? o.h | 0 : MILITARY_OUTPOST_SIZE;
    if (inRectSized(o.x0, o.y0, mw)) return true;
  }
  return false;
}

/**
 * Изолированные группы по всем командам: компоненты без 8-связного пути **ни к главной базе, ни к любому плацдарму**.
 * Сторона главной базы берётся из `team.spawnS` (1…{@link FLAG_SPAWN_SIZE}), иначе {@link FLAG_SPAWN_SIZE}.
 *
...
export function computeIsolatedTerritoryGroups(pixels, teams, pixelTeamFn, flagCellFromSpawnFn) {
  ...
  for (const t of teams) {
    ...
    const spawnSize =
      typeof t.spawnS === "number" && Number.isFinite(t.spawnS) && t.spawnS >= 1
        ? Math.min(FLAG_SPAWN_SIZE, Math.max(1, t.spawnS | 0))
        : FLAG_SPAWN_SIZE;

    const reachable = computeSupplyReachableFromTeamBases(
      vertices,
      t.spawnX0,
      t.spawnY0,
      t.militaryOutposts,
