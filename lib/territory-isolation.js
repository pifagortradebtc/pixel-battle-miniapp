 ...
 *
 * **Изоляция и коллапс через ~20 с** (`computeIsolatedTerritoryGroups`): «снабжение» от **главной** базы 6×6 **и**
 * от всех **военных плацдармов** (2×2). Карман и таймер только если участок не связан ни с штабом, ни с купленной базой.
 *
 * **Размещение пикселей** на сервере — BFS от главной и плацдармов (`computeBaseConnectedPixelKeysForTeam`), в том же духе.
 *
 * Внутри главной базы 6×6 или плацдарма клетки не попадают в список кармана — см. `cellKeyInsideAnyTeamBaseRect`.
 */

import { MILITARY_OUTPOST_SIZE } from "./flag-capture.js";
 ...
 * Для расчёта **только от главной** (без плацдармов) передайте `militaryOutposts: []` — см. `computeSupplyReachableForIsolation`.
 *
 * @param {Set<string>} vertices
...
/**
 * Достижимость **только от главной** 6×6 — единственный корень для таймера изоляции и коллапса (~20 с).
 * Передовые базы намеренно не семенят BFS (передаётся пустой список плацдармов).
 *
 * @param {Set<string>} vertices
 * @param {number} spawnX0
 * @param {number} spawnY0
...
/**
 * Изолированные группы по всем командам: компоненты без 8-связного пути **ни к главной** 6×6, **ни к военному плацдарму**.
 *
