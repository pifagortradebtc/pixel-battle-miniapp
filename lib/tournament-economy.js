...
export const PRICES_QUANT = {
  personal: { 15: 14, 10: 28, 5: 56, 2: 112 },
  team: { 15: 140, 10: 210, 5: 350, 2: 700 },
  zone4: 84,
  zone6: 168,
};
...
export function getEffectiveRecoverySec(user, teamFx, now = Date.now()) {
  ...
  let p = base;
  if (
    user.personalRecoveryUntil > now &&
    typeof user.personalRecoverySec === "number" &&
    user.personalRecoverySec >= 2
  ) {
    p = user.personalRecoverySec;
  }
  let t = base;
  if (
    teamFx.teamRecoveryUntil > now &&
    typeof teamFx.teamRecoverySec === "number" &&
    teamFx.teamRecoverySec >= 2
  ) {
    t = teamFx.teamRecoverySec;
  }
