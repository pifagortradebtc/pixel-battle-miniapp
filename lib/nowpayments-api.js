 ...
 *
 * Сумма `amountUsd` — число в «долларах США» в терминах NOWPayments (1:1 со стейблкоином USDT).
 * Для оплаты в USDT на BSC используйте pay_currency: usdtbsc (BEP20).
 *
...
export async function createNowpaymentInvoice(p) {
  ...
  const priceCur = String(p.priceCurrency || "usd").trim().toLowerCase();
