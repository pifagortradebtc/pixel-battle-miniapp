 ...
 *
 * @param {{ apiKey: string, amountUsd: number, orderId: string, ipnUrl: string, successUrl?: string, cancelUrl?: string, payCurrency?: string, apiBase?: string, orderDescription?: string }} p
 */
export async function createNowpaymentInvoice(p) {
  ...
  const res = await fetch(`${apiBase}/invoice`, {
    ...
    body: JSON.stringify({
      ...
      order_id: p.orderId,
      order_description: p.orderDescription || "Pixel Battle — пополнение баланса",
