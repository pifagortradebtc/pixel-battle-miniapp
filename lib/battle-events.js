  ...
  for (let i = 0; i < timeline.length; i++) {
    ...
    switch (ev.eventType) {
      case "gold_zone": {
        ...
        const lk =
          typeof ev.payload?.layerKind === "string" ? ev.payload.layerKind : "gold_zone";
        const rectKey = `${ev.eventId}`;
        snap.goldRect = pickLandRectangle(rectKey, roundIndex, play, gridW, gridH, landGrid, zs);
        snap.goldUntilMs = eventWindowEndMs(ev, play, end);
        snap.goldUi = {
          title: ev.uiTitle,
      ...
      case "economic_mixed": {
        ...
        snap.economicUntilMs = eventWindowEndMs(ev, play, end);
        snap.economicUi = {
          title: ev.uiTitle,
          subtitle: ev.uiSubtitle,
      ...
      case "team_synergy": {
        ...
        snap.synergyUntilMs = eventWindowEndMs(ev, play, end);
        break;
      }
