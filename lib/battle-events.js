...
export const ROUND_BATTLE_EVENT_TIMELINES = /** @type {RoundTimelineEventDef[][]} */ ([
  // ——— Round 0 (mass): бой 5 h после разминки — каждый тип события хотя бы раз, без выхода за конец боя.
  [
    {
    ...
    {
      eventId: "r0_dramatic_mid",
      eventType: "dramatic_pressure",
      startOffsetMs: 152 * MIN,
      durationMs: 22 * MIN,
    ...
    },
    {
      ...
      durationMs: 0,
      warnLeadMs: 4500,
      uiTitle: "Сейсмическая активность",
      uiSubtitle: "Часть территорий обрушилась",
      payload: { aftermathMs: 20_000 },
    },
    {
      eventId: "r0_map_compression",
      eventType: "map_compression",
      startOffsetMs: 198 * MIN,
      durationMs: null,
...
Автотаймлайн раунда отключён: золото, экономика, сжатие карты и т.д. только после вашей команды.
По умолчанию каждое событие длится 20 минут (или до конца боя, если раньше). Своя длительность: gold 45 (минут).
...
export function computeBattleScoringSnapshot(nowMs, ctx) {
  ...
  /* События карты (золото, экономика, сжатие, синергия, баннеры) — только через ручные слоты mergeManualBattleSlotsIntoSnapshot, не по часам таймлайна. */
