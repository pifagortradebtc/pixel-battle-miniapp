 ...
 * раскиданных по сторонам: в каждом из 8 направлений — как можно дальше от центра острова.
 ...
 * по одному кандидату в каждом из 8 угловых секторов от центра масс острова — с максимальной
 * удалённостью от центра; недостающие добираются жадно по убыванию расстояния.
...
export function computeQuantumFarmLayouts(playableGrid, gridW, gridH) {
  ...
  /** @type {{ x0: number, y0: number, d: number, sector: number }[]} */
  const cands = [];
  for (let y0 = 0; y0 <= gridH - QUANTUM_FARM_SIZE; y0++) {
    for (let x0 = 0; x0 <= gridW - QUANTUM_FARM_SIZE; x0++) {
      ...
      cands.push({ x0, y0, d, sector });
    }
  }
  if (!cands.length) return [];

  /** @type {Map<number, { x0: number, y0: number, d: number }[]>} */
  const bySector = new Map();
  for (let s = 0; s < QUANTUM_FARM_COUNT; s++) bySector.set(s, []);
  for (let i = 0; i < cands.length; i++) {
    const c = cands[i];
    const arr = bySector.get(c.sector);
    if (arr) arr.push({ x0: c.x0, y0: c.y0, d: c.d });
  }
  ...
  if (picked.length < QUANTUM_FARM_COUNT) {
    const byDist = cands.slice().sort((a, b) => b.d - a.d);
