...
/**
 * Слить economy_state из БД в кэшированного пользователя без потери более свежих значений в RAM
 * (покупка баффа → debounce flush: reload не должен откатить personalRecoveryUntil к старому json).
 * @param {EconomyUser} target
 * @param {Record<string, unknown>} econ
 */
export function mergeEconomyJsonIntoCachedUser(target, econ) {
  if (!target || !econ || typeof econ !== "object") return;
  if (typeof econ.lastActionAt === "number" && Number.isFinite(econ.lastActionAt)) {
    target.lastActionAt = Math.max(target.lastActionAt | 0, econ.lastActionAt | 0);
  }
  const nzMax = (a, b) => {
    const x = Number(a);
    const y = Number(b);
  ...
  if (typeof econ.lastZoneCaptureAt === "number" && Number.isFinite(econ.lastZoneCaptureAt)) {
    target.lastZoneCaptureAt = nzMax(target.lastZoneCaptureAt, econ.lastZoneCaptureAt);
  }
  if (typeof econ.lastMassCaptureAt === "number" && Number.isFinite(econ.lastMassCaptureAt)) {
    target.lastMassCaptureAt = nzMax(target.lastMassCaptureAt, econ.lastMassCaptureAt);
  }
  if (typeof econ.lastZone12CaptureAt === "number" && Number.isFinite(econ.lastZone12CaptureAt)) {
    target.lastZone12CaptureAt = nzMax(target.lastZone12CaptureAt, econ.lastZone12CaptureAt);
  }
  const dbU = Number(econ.personalRecoveryUntil);
  const memU = Number(target.personalRecoveryUntil);
  const dbV = Number.isFinite(dbU) ? dbU : 0;
  const memV = Number.isFinite(memU) ? memU : 0;
  if (dbV > memV) {
    target.personalRecoveryUntil = dbV;
    if (typeof econ.personalRecoverySec === "number" && Number.isFinite(econ.personalRecoverySec)) {
      target.personalRecoverySec = econ.personalRecoverySec;
    }
  }
  if (typeof econ.invitedByPlayerKey === "string" && econ.invitedByPlayerKey && !target.invitedByPlayerKey) {
    target.invitedByPlayerKey = econ.invitedByPlayerKey;
  }
  migrateEconomyUser(target);
}
