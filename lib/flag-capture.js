...
/** Магазин «Ремонт базы»: прирост HP за одно применение (не выше max базы). */
export const BASE_REPAIR_HP_DELTA = 20;
...
/**
 * Координаты клетки флага (центр квадратной главной базы стороны `spawnSide`), те же что destroyGx на сервере.
 * @param {number} spawnX0
 * @param {number} spawnY0
 * @param {number} [spawnSide] по умолчанию {@link FLAG_SPAWN_SIZE}
 */
export function flagCellFromSpawn(spawnX0, spawnY0, spawnSide = FLAG_SPAWN_SIZE) {
  const ox = spawnX0 | 0;
  const oy = spawnY0 | 0;
  const S = Math.max(1, spawnSide | 0);
  return {
    x: ox + Math.floor(S / 2),
    y: oy + Math.floor(S / 2),
  };
